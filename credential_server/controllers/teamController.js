...
exports.inviteMember = async (req, res) => {
  try {
    ...
    if (team.leader.toString() !== leaderId)
      return res
        .status(403)
        .json({ message: "Only the leader can invite members" });
    ...
    if (
      invitedUser &&
      team.members.map((m) => m.toString()).includes(invitedUser._id.toString())
    )
...
exports.acceptInvite = async (req, res) => {
  try {
    ...
    if (invite.email !== user.email.toLowerCase())
      return res
        .status(403)
        .json({ message: "This invitation is for a different email address" });
